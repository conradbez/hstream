...
class Components:
    ...
    def build_component(self, component_type, label=None, default_value=None, key=None, **kwargs):
    ...
    def component_wrapper(component_fucntion):
        ...
        def wrapped_component_function(self, *method_args, **method_kwargs):
            ...
            value = self.build_component(**component_fucntion(self,  *method_args, **method_kwargs)) 
            return value
    ...
    def markdown(self, text: str, *, key: str = False) -> None:
        ...
        with self.tag('div'):
            self.doc.asis(html)
        display_html = self.return_old_doc_and_init_new()
        return dict(label=display_html, component_type='Markdown', key=key, )
    ...
    def text_input(self, label: str, default_value: str, key:str = None) -> str:
        ...
        return dict(label=html, component_type='TextInput', default_value=default_value, key=key)
    ...
    def slider(self, label: str, minValue:int, maxValue:int, default_value: int, key:str = None) -> str:
        ...
        with self.tag('input',
                    ...
                    self.text('')
        html = self.return_old_doc_and_init_new()
        return dict(label=html, component_type='Slider', default_value=default_value, key=key, **kwargs)
        ...
        for item in label:
            with self.tag('ul'):
                with self.tag('li',):
                    with self.tag('a',
                        ('href', '#'),
                        ('hx-post',  f"/value_changed/{component_key}?{component_key}={item}"),
                    ):
                        self.text(str(item))
        html = self.return_old_doc_and_init_new()
        return dict(label=html, component_type='Nav', default_value=default_value, key=key, **kwargs)
