...
from fastapi import FastAPI, Request, WebSocket, Cookie
...
class Hyperstream(Components):
    ...
    def __call__(self):
        ...
        async def evaluate_user_code_middleware(request: Request, call_next,):
            ...
            if not hs_user_id:
                ...
                context.hs_user_app_db_path = self.path_to_usesr_directory / str(hs_user_id)
    ...
    def build_fastapi_app(self):
        ...
        async def root(request: Request, response: Response, ):
            ...
            response = templates.TemplateResponse("main.html", {"request": request, "components": components},)
            return response
    
        @self.app.get("/{component_key}/label", response_class=HTMLResponse)
        async def func_for_component(component_key,request: Request, response: Response):
            ...
            self.clear_component_refresh_queue(component = component_attr['component_key'])
    ...
    def compile_user_code(self):
        ...
        filebody = f"""builtins.hs_user_app_db_path = "{getattr(context, 'hs_user_app_db_path', 'error')}" \n"""+filebody
    ...
    def run_user_script(self):
        assert context.hs_user_app_db_path # we always need the visitor's path before we execute the script so we know where to store the visitors components
